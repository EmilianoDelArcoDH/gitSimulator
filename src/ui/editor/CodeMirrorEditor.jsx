...
import * as EmmetCM from "@emmetio/codemirror6-plugin";
...
function languageExtensions(language) {
  switch (language) {
    case "html":
      return [htmlLang(), EmmetCM.emmetPlugin ? EmmetCM.emmetPlugin() : null].filter(Boolean);
    case "css":
      return [cssLang(), EmmetCM.emmetPlugin ? EmmetCM.emmetPlugin({ syntax: "css" }) : null].filter(Boolean);
    case "javascript":
      return [jsLang({ jsx: true, typescript: false })];
    case "markdown":
      return []; // basic text, could add markdown lang if needed
    default:
      return [];
  }
}
